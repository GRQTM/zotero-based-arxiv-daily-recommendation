...
## Recommendation Engine (Codex Direct)
- Codex directly reads your profile and the latest arXiv paper list, then selects the top 5 recommendations.
- Paper summaries and recommendation reasons are also generated by Codex at runtime.
...
- You can override it per run with `MODEL_REASONING_EFFORT`.
- Allowed values: `low`, `medium`, `high`

Examples:
```bash
MODEL_REASONING_EFFORT=medium ./start
MODEL_REASONING_EFFORT=low ./start -r
```

This value is applied to both Codex steps:
- profile generation (`./start -r`)
- paper recommendation (`./start`, `./start -r`)
